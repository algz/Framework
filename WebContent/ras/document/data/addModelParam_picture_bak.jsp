	...
	*/
	function createFileinput(id, photo) {
			...
			$(id).fileinput({
				...
				//showPreview:true, //是否开启预览.默认true.
				deleteUrl : './delpicturefile',//删除URL.string, the URL for deleting the image/content in the initial preview via AJAX post response. This will be overridden by the initialPreviewConfig['url'] property.
				...
				layoutTemplates:{
					footer:  '<input type="text" style="width:120px;" value="{tag}" placeholder="标签名">'+
	                '<div class="file-thumbnail-footer">\n' +
	                '    <div class="file-footer-caption" title="{caption}">{caption}<br>{size}<br>'+
	                '</div>\n' +
	                '    {progress} {actions}\n' +
	                '</div>'
				}
