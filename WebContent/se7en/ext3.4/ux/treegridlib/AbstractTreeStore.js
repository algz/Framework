...
Ext.ux.maximgb.tg.AbstractTreeStore = Ext.extend(Ext.data.Store, {
	...
	load : function(options) {
		...
		if (options.params[this.paramNames.active_node] !== null) {
			...
			{
			options.add = false; //2012年7月7日 修改,将把加载的记录不添加到当前的缓存
 			}
	...
	loadRecords : function(o, options, success) {
        ...
        //表格tr鼠标滑过样式
        $(".uiframe-treeGridTr").hover(function(){
            $(this).addClass("uiframe-treeGridTr-hover");
        },function(){
            $(this).removeClass("uiframe-treeGridTr-hover");
        });
	},
