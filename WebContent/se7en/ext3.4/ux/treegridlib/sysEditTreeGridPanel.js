	...
	initTemplates : function()
    {
        ...
        if (!ts.row) {
            ts.row = new Ext.Template(
                '<div class="x-grid3-row uiframe-border ux-maximgb-tg-level-{level} {alt} uiframe-treeGridTr" style="{tstyle} {display_style}">',
        ...
        if(!ts.iconCls){
        	ts.iconCls = new Ext.Template(
	        	'<div unselectable="on" class="x-tree-node-icon {imgCls}" style="width:16px;height:16px;float:left;"></div>'
        	);
        }
        this.templates = ts;
...
//            left = 0;
        ...
        if(record.data["iconCls"]&&record.data["iconCls"]!=""){
	        tpl_data.iconCls = iconCls.apply({
	        	...
	        	treeIcon : basePath + record.data["iconCls"]
	        });
