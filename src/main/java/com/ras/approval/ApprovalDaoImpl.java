...
//			sf.getCurrentSession().saveOrUpdate(approval);
		...
		//1审批同意
		if(approval.getDataTable()!=null&&approval.getApprovalResult().equals("1")){
			String sql="update RAS_AIRCRAFT_"+approval.getDataTable()+" set PERMISSION_LEVEL='"+approval.getPermissionLevel()+"' "
					+ " where id='"+approval.getDataID()+"'";
